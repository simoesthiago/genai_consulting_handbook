...
**Prerequisites:**
- [`2.7_agentic_ai.md`](./2.7_agentic_ai.md) - Understanding agentic loops and autonomy
- [`2.2_tool_calling.md`](./2.2_tool_calling.md) - Tool gateway and enforcement patterns
- [`2.4_guardrails.md`](./2.4_guardrails.md) - Safety boundaries for multiple acting entities

**Related:**
- [`2.3_mcp.md`](./2.3_mcp.md) - Standardizing tool access for multiple agents
- [`2.6_observability_llomps.md`](./2.6_observability_llomps.md) - Tracing coordination and handoffs
...
```mermaid
flowchart TD
    User([User / App]) --> Coordinator["<b>COORDINATOR</b><br/>(Supervisor)"]
    
    subgraph Specialists["SPECIALIST AGENTS"]
        direction LR
        Retrieval["<b>Retrieval Agent</b><br/>(RAG + Citations)"]
        Action["<b>Action Agent</b><br/>(Tool Proposals)"]
        Compliance["<b>Compliance Agent</b><br/>(Policy Checks)"]
        Reporter["<b>Report Agent</b><br/>(Formatting)"]
    end

    Coordinator --> Specialists
    Specialists --> Coordinator

    Coordinator --> Gateway["<b>TOOL GATEWAY</b><br/>(Enforcement)"]
    Gateway --> Systems[(Enterprise Systems)]
    
    subgraph StateLayer["STATE & GOVERNANCE"]
        State[(Shared Task State)]
        Audit[(Audit Logs)]
    end

    Coordinator -.-> StateLayer
    Gateway -.-> Audit
```
