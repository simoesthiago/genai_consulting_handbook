...
**Prerequisites:**
- [`2.2_tool_calling.md`](./2.2_tool_calling.md) - Tool calling mechanics and safety
- [`2.4_guardrails.md`](./2.4_guardrails.md) - Enforcement layers for autonomous actions
- [`2.5_evals.md`](./2.5_evals.md) - Measuring success in iterative loops

**Related:**
- [`2.8_multi_agent_system.md`](./2.8_multi_agent_system.md) - Coordinating between specialized agents
- [`2.6_observability_llomps.md`](./2.6_observability_llomps.md) - Tracing complex agentic workflows
- [`3.4_process_automation.md`](../03_use_case_archetypes/3.4_process_automation.md) - Agentic AI in automation archetypes
...
```mermaid
graph LR
    L0["<b>L0: Assistant</b><br/>Answer-only<br/>No tools"] --> L1["<b>L1: Advisor</b><br/>Suggest actions<br/>Human executes"]
    L1 --> L2["<b>L2: Executor</b><br/>Low-risk tools<br/>Read-only/Reversible"]
    L2 --> L3["<b>L3: Delegate</b><br/>Executes with<br/>Approval gates"]
    L3 --> L4["<b>L4: Autonomous</b><br/>Full execution<br/>Evidence-based"]
    
    style L0 fill:#f4f4f4,stroke:#333
    style L4 fill:#e1f5fe,stroke:#01579b
```
...
```mermaid
flowchart TD
    User([User Message]) --> Norm["<b>NORMALIZE</b><br/>(Risk labeling + Intent)"]
    Norm --> State["<b>BUILD CONTEXT</b><br/>(State + RAG)"]
    State --> LLM["<b>LLM PROPOSAL</b><br/>(Next step / Plan)"]
    LLM --> Val["<b>VALIDATE</b><br/>(Policy / Schema)"]
    Val --> Gateway{Gateway}
    Gateway -- Approval Required --> UserApproval([Wait for Human])
    UserApproval --> Execute
    Gateway -- Authorized --> Execute["<b>EXECUTE TOOLS</b>"]
    Execute --> Record["<b>UPDATE STATE</b><br/>(Record Outcomes)"]
    Record --> Done{Done?}
    Done -- No --> State
