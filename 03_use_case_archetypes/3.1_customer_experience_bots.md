...
**Prerequisites:**
- [1.1 LLM Fundamentals](../01_foundations/1.1_llm_fundamentals.md) - Understanding model capabilities and limitations
- [1.3 Hallucinations Basics](../01_foundations/1.3_hallucinations_basics.md) - Critical for customer-facing responses

**Related:**
- [3.2 Internal GenAI Copilots](./3.2_genai_copilots_internal.md) - Similar pattern for internal employee audiences
...
```mermaid
quadrantChart
    title CX Bot Risk Heat Map
    x-axis Low Frequency --> High Frequency
    y-axis Low Impact --> High Impact
    quadrant-1 Monitor Closely
    quadrant-2 Critical Priority
    quadrant-3 Accept / Mitigate
    quadrant-4 Prevent Proactively
    "Truth Risk (Hallucination)": [0.7, 0.85]
    "Data Risk (PII Leak)": [0.3, 0.95]
    "Action Risk (Wrong Tool)": [0.4, 0.8]
    "Compliance Risk": [0.25, 0.75]
    "Reputational Risk": [0.6, 0.6]
```
...
## 2.3. As-Is user journey (where friction lives)

The following diagram shows a typical customer support journey today, highlighting where friction accumulates:

```mermaid
flowchart TB
    subgraph Customer Journey
        C1[Customer has issue] --> C2[Searches FAQ/Help]
        C2 --> C3{Found answer?}
        C3 -->|No| C4[Initiates contact]
        C3 -->|Partial| C4
        C4 --> C5[Waits in queue]
        C5 --> C6[Explains issue to agent]
    end
    
    subgraph Agent Journey
        C6 --> A1[Agent searches KB]
        A1 --> A2[Checks CRM/systems]
        A2 --> A3{Found answer?}
        A3 -->|No| A4[Asks colleague/escalates]
        A3 -->|Yes| A5[Provides response]
        A4 --> A5
    end
    ...
    subgraph Pain Points
        ...
        P2[❌ Long wait times]
        P3[❌ Context lost]
        P4[❌ Inconsistent answers]
        P5[❌ Rework and re-search]
    end
...
```mermaid
flowchart LR
    subgraph ASSIST["🟢 ASSIST (MVP Default)"]
        A1[Answer questions]
        A2[Guide troubleshooting]
        A3[Summarize policies]
        A4[Clarify intent]
    end
    
    subgraph AUTOMATE["🟡 AUTOMATE (Selective)"]
        B1[Create ticket]
        B2[Check order status]
        B3[Schedule callback]
        B4[Password reset]
    end
    
    subgraph DECIDE["🔴 DECIDE (Human Approval)"]
        C1[Refunds/credits]
        C2[Plan changes]
        C3[Account closures]
        C4[Policy exceptions]
    end
    
    ASSIST -->|"Earn trust + prove safety"| AUTOMATE
    AUTOMATE -->|"Strong controls + evidence"| DECIDE
```
...
```mermaid
flowchart TD
    Start[Bot receives message] --> Classify{Intent + Risk<br/>Classification}
    
    Classify -->|In-scope, low risk| Retrieve[Retrieve Evidence]
    Classify -->|Out-of-scope| Escalate
    Classify -->|High-risk action| Escalate
    
    Retrieve --> Evidence{Evidence<br/>found?}
    Evidence -->|Yes, confident| Respond[Generate Response<br/>with Citations]
    Evidence -->|No/Low confidence| Abstain[Abstain + Clarify]
    
    Respond --> Guardrails{Passes<br/>guardrails?}
    Guardrails -->|Yes| Deliver[Deliver to Customer]
    Guardrails -->|No| Escalate
    
    Abstain --> Loop{2nd attempt<br/>failed?}
    Loop -->|No| Start
    Loop -->|Yes| Escalate
    
    Escalate[Escalate to Agent]
    Escalate --> Summary[Generate Summary:<br/>• Intent + entities<br/>• Steps tried<br/>• Evidence gaps<br/>• Recommended action]
    Summary --> Handoff[Route to Agent Queue]
    
    style Escalate fill:#f9f,stroke:#333
    style Handoff fill:#9f9,stroke:#333
```
...
Use this decision tree to quickly evaluate whether the CX Bot archetype is appropriate:

```mermaid
flowchart TD
    Start[Evaluate CX Bot Fit] --> Q1{High-volume<br/>repeatable intents?}
    
    Q1 -->|No| Anti1[❌ Anti-fit:<br/>Low ROI]
    Q1 -->|Yes| Q2{Source of truth<br/>for policies exists?}
    
    Q2 -->|No| Anti2[❌ Anti-fit:<br/>Fix knowledge first]
    Q2 -->|Yes| Q3{Can separate<br/>auth vs unauth?}
    
    Q3 -->|No| Anti3[❌ Anti-fit:<br/>Security risk]
    Q3 -->|Yes| Q4{Willing to<br/>abstain + escalate?}
    
    Q4 -->|No| Anti4[❌ Anti-fit:<br/>Governance gap]
    Q4 -->|Yes| Q5{Escalation workflow<br/>exists?}
    
    Q5 -->|No| Partial[⚠️ Build escalation<br/>before bot]
...
```mermaid
gantt
    title CX Bot Delivery Timeline
    dateFormat YYYY-MM-DD
    section POC/MVP
        Discovery + Scope           :a1, 2025-01-01, 2w
        RAG + Citations             :a2, after a1, 2w
        Handoff + Guardrails        :a3, after a2, 2w
        Eval Gates + Sign-off       :milestone, after a3, 0d
    section Pilot
        Controlled Rollout          :b1, after a3, 2w
        Intent Expansion            :b2, after b1, 2w
        Routing + QA Loop           :b3, after b2, 2w
        Pilot Gate                  :milestone, after b3, 0d
    section Scale
        Multi-channel               :c1, after b3, 4w
        Localization + Tools        :c2, after c1, 4w
        Cost Optimization           :c3, after c2, 4w
        Full Production             :milestone, after c3, 0d
```
...
## 7.1. Architecture choice and why it fits
...
## 7.2. Delivery timeline (phases and gates)
...
## 7.3. Eval plan and go/no-go evidence
...
## 7.5. A real incident and how it became a regression gate
